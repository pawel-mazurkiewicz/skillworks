...
- Always run `npm test` and `npm run build` before committing UI changes. `npm run test:ui` runs the Playwright smoke suite (`test/ui/`) against Chromium headless, driven by `playwright.config.js`'s `webServer` (`npm run dev`); requires a one-time `npx playwright install chromium` on the machine. It mocks the `/api/**` command layer via `page.route()`, so it needs no backend process — see `test/ui/mcp-servers.spec.js` for the pattern.
